...
def softmax(x):
    ...
    rowmax = tf.reshape(tf.reduce_max(x, 1),[-1,1]) # reshape tensor for broadcasting
    rowsum = tf.reshape(tf.reduce_sum(tf.exp(x - rowmax), 1),[-1,1])
    out = tf.exp(x - rowmax)/rowsum
    ### END YOUR CODE
    return out
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    yint = tf.to_float(y, name='ToFloat')
    # yhatfloat = tf.to_float(y, name='ToFloat')
    out = tf.reduce_sum(-yint*tf.log(yhat))
    ### END YOUR CODE
    return out
