...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        # length = min([len(unfinished_parses),batch_size])
        # minibatch = unfinished_parses[:length]
        minibatch = unfinished_parses[:batch_size]
        next_transition = model.predict(minibatch)
        rm = []

        for i in range(len(next_transition)):
            minibatch[i].parse_step(next_transition[i])
            if len(minibatch[i].buffer) == 0 and len(minibatch[i].stack) == 1:
                rm.append(i)

        # for k in rm:
        #     dependencies.append(unfinished_parses[k].dependencies)
