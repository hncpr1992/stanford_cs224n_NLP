...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentenceIndex = [tokens[w] for w in sentence]
    sentenceVector = np.average(wordVectors[sentenceIndex,:], axis = 0)
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    for i in range(len(results)):
    	if results[i]["test"] > maxtest:
    		maxtest = results[i]["test"]
    		dindex = i
    bestResult = results[dindex]
    ### END YOUR CODE
