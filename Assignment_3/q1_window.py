...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
    	prepadding = [start]*window_size
    	postpadding = [end]*window_size
    	sentence_pad = prepadding + sentence + postpadding
    	for i in range(len(sentence)):
    		windowed_data.append(([y for x in sentence_pad[i:(i+(2*window_size+1))] 
    								 for y in x ],labels[i]))
    ### END YOUR CODE
