...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) >= max_length:
	        ...
	        sentence_pad = sentence + [zero_vector]*(max_length-len(sentence))
	        labels_pad = labels + [zero_label]*(max_length-len(sentence))
	        ret.append([sentence_pad, labels_pad, masking])
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable(name="b2",
        					 ...
        					 initializer = xavier_initializer)
        h_t = tf.zeros(shape=(1, self.config.hidden_size),name = "h_t")
        ### END YOUR CODE
            ...
            for time_step in range(self.max_length):
                ...
                if time_step == 0:
                    with tf.variable_scope("RNNCell"):
                        tf.get_variable("W_x", initializer=np.array(np.eye(cell.input_size,cell.state_size), dtype=np.float32))
                        tf.get_variable("W_h", initializer=np.array(np.eye(cell.state_size,cell.state_size), dtype=np.float32))
                        tf.get_variable("b",  initializer=np.array(np.ones(cell.state_size), dtype=np.float32))
                    tf.get_variable_scope().reuse_variables()
                o_t, h_t = cell(x[:,time_step], h_t, scope="RNNCell")
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        raw_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss_mask = tf.boolean_mask(raw_loss, self.mask_placeholder)
        loss = tf.reduce_mean(loss_mask)
